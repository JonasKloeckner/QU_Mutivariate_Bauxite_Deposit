...
def Run_Vmodel_NS_ATGc():
    parfile = os.path.join('4-Variograms', 'NS_Model', 'NS_ATGc.par')
    f = open(parfile, 'w')
    f.write('''            Parameters for VMODEL
                  *********************

START OF PARAMETERS:
4-Variograms/NS_Model/NS_ATGc.var                   -file for variogram output
...
3    0.10                     -nst, nugget effect
1    0.55  0.0   0.0   0.0    -it,cc,ang1,ang2,ang3
         60.0   60.0  1.50    -a_hmax, a_hmin, a_vert
1    0.15  0.0   0.0   0.0    -it,cc,ang1,ang2,ang3
         500.0   500.0  1.70    -a_hmax, a_hmin, a_vert         
1    0.20  0.0   0.0   0.0    -it,cc,ang1,ang2,ang3
         5000.0   5000.0  2.5    -a_hmax, a_hmin, a_vert
''')
    f.close()
    exe_file = os.path.join('Gslib', 'vmodel')
    subprocess.call([exe_file, parfile], shell=True)

def Run_Vmodel_NS_STGc():
    parfile = os.path.join('4-Variograms', 'NS_Model', 'NS_STGc.par')
    f = open(parfile, 'w')
    f.write('''            Parameters for VMODEL
                  *********************

START OF PARAMETERS:
4-Variograms/NS_Model/NS_STGc.var                   -file for variogram output
2   400                       -number of directions and lags
0.0   -90.0    0.1            - azm, dip, lag distance (vertical direction)
0.0   0.0      5.0            - azm, dip, lag distance (horizontal direction)
3    0.0                     -nst, nugget effect
1    0.45  0.0   0.0   0.0    -it,cc,ang1,ang2,ang3
         55.0   55.0  2.20    -a_hmax, a_hmin, a_vert
1    0.18  0.0   0.0   0.0    -it,cc,ang1,ang2,ang3
         250.0   250.0  2.30    -a_hmax, a_hmin, a_vert
1    0.37  0.0   0.0   0.0    -it,cc,ang1,ang2,ang3
    2200.0   2200.0  2.40    -a_hmax, a_hmin, a_vert
''')
    f.close()
    exe_file = os.path.join('Gslib', 'vmodel')
    subprocess.call([exe_file, parfile], shell=True)

def Run_Vmodel_NS_FEGc():
    parfile = os.path.join('4-Variograms', 'NS_Model', 'NS_FEGc.par')
    f = open(parfile, 'w')
    f.write('''            Parameters for VMODEL
                  *********************

START OF PARAMETERS:
4-Variograms/NS_Model/NS_FEGc.var                   -file for variogram output
2   400                       -number of directions and lags
0.0   -90.0    0.1            - azm, dip, lag distance (vertical direction)
0.0   0.0      5.0            - azm, dip, lag distance (horizontal direction)
...
def Run_Vmodel_NS_TIGc():
    parfile = os.path.join('4-Variograms', 'NS_Model', 'NS_TIGc.par')
    f = open(parfile, 'w')
    f.write('''            Parameters for VMODEL
                  *********************

START OF PARAMETERS:
4-Variograms/NS_Model/NS_TIGc.var                   -file for variogram output
2   400                       -number of directions and lags
0.0   -90.0    0.1            - azm, dip, lag distance (vertical direction)
0.0   0.0      5.0            - azm, dip, lag distance (horizontal direction)
...
def Run_Vmodel_NS_AAG():
    parfile = os.path.join('4-Variograms', 'NS_Model', 'NS_AAG.par')
    f = open(parfile, 'w')
    f.write('''            Parameters for VMODEL
                  *********************

START OF PARAMETERS:
4-Variograms/NS_Model/NS_AAG.var                   -file for variogram output
2   400                       -number of directions and lags
0.0   -90.0    0.1            - azm, dip, lag distance (vertical direction)
0.0   0.0      5.0            - azm, dip, lag distance (horizontal direction)
3   0.10                     -nst, nugget effect
1    0.59  0.0   0.0   0.0    -it,cc,ang1,ang2,ang3
         60.0   60.0  1.25    -a_hmax, a_hmin, a_vert
1    0.21  0.0   0.0   0.0    -it,cc,ang1,ang2,ang3
         550.0   550.0  7.00    -a_hmax, a_hmin, a_vert     
1    0.10  0.0   0.0   0.0    -it,cc,ang1,ang2,ang3
         1500.0   1500.0  9.00    -a_hmax, a_hmin, a_vert     
''')
    f.close()
    exe_file = os.path.join('Gslib', 'vmodel')
    subprocess.call([exe_file, parfile], shell=True)

def Run_Vmodel_NS_SRG():
    parfile = os.path.join('4-Variograms', 'NS_Model', 'NS_SRG.par')
    f = open(parfile, 'w')
    f.write('''            Parameters for VMODEL
                  *********************

START OF PARAMETERS:
4-Variograms/NS_Model/NS_SRG.var                   -file for variogram output
2   400                       -number of directions and lags
0.0   -90.0    0.1            - azm, dip, lag distance (vertical direction)
0.0   0.0      5.0            - azm, dip, lag distance (horizontal direction)
3    0.0                     -nst, nugget effect
1    0.35  0.0   0.0   0.0    -it,cc,ang1,ang2,ang3
         65.0   65.0  2.00    -a_hmax, a_hmin, a_vert
1    0.20  0.0   0.0   0.0    -it,cc,ang1,ang2,ang3
         230.0   230.0  2.20    -a_hmax, a_hmin, a_vert
1    0.45  0.0   0.0   0.0    -it,cc,ang1,ang2,ang3
    2000.0   2000.0  2.40    -a_hmax, a_hmin, a_vert
''')
    f.close()
    exe_file = os.path.join('Gslib', 'vmodel')
    subprocess.call([exe_file, parfile], shell=True)
...
if __name__ == '__main__':
    ...
    names = ['NS_ATGc', 'NS_STGc', 'NS_FEGc', 'NS_TIGc', 'NS_PPGc', 'NS_AAG', 'NS_SRG' ]
